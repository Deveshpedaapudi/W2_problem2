...
Vars=evalin('base','whos(''W2_prob2'')');
...
if ~isempty(Vars) && ~isempty(Value),  
  ...
  for lp=1:10,          
      ...
      y_data=evalin('base',['W2_prob2.get(''', PlotMe ''')', '.Values.Data']);        
      x_data=evalin('base',['W2_prob2.get(''', PlotMe ''')', '.Values.Time']);        
...
if ~isempty(find_system(0,'flat','name','W2_prob2')) && ...
   strcmp(get_param('W2_prob2','Open'),'off'),
  return
end
...
Name=find_system('W2_prob2','SearchDepth',1,'BlockType','Outport');
...
TempInputName=find_system('W2_prob2'     , ...
                          'SearchDepth',1, ...
...
ModelPos=get_param('W2_prob2','Location');
